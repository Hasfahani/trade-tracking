...
def detect_interesting_activity(db: Session) -> List[Dict[str, Any]]:
    ...
    Categories:
    - large_trade: single trade whose price x size >= _LARGE_TRADE_THRESHOLD.
    ...
    The query budget stays bounded for dashboard rendering: load the recent
    activity window once, then check whether those wallet/market pairs existed
    before the window.
    """
    ...
    timestamps_by_wallet: Dict[str, List[datetime]] = {}
    for trade in recent_trades:
        timestamps_by_wallet.setdefault(trade.wallet_address, []).append(trade.traded_at)

    for wallet_address, timestamps in timestamps_by_wallet.items():
    ...
    # Check recent wallet/market pairs against older history.
    recent_pairs = {(trade.wallet_address, trade.condition_id) for trade in recent_trades}
    prior_pairs = set()
    if recent_pairs:
        wallets = {wallet for wallet, _ in recent_pairs}
        conditions = {condition for _, condition in recent_pairs}
        prior_pairs = {
            (row.wallet_address, row.condition_id)
            for row in (
                db.query(Trade.wallet_address, Trade.condition_id)
                .filter(
                    Trade.wallet_address.in_(wallets),
                    Trade.condition_id.in_(conditions),
    ...
    for trade in sorted(recent_trades, key=lambda t: t.traded_at):
        key = (trade.wallet_address, trade.condition_id)
        if key in prior_pairs or key in rep_trades:
            continue
